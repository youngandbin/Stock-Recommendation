...
class GraphEmbedding(EmbeddingModule):
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
    ...
    # 0. 처음엔 memory + node features 으로 초기화
    if n_layers == 0:
      return source_node_features
    
    # 그 다음부터 propagation
    else:
      # 1. source_nodes(=sources, destinations, negatives)의 임베딩 구하기
      ...
      # 2. neighbors 찾기
      # neighbor finder: 
      # train_data의 source, destination 저장된 adj_list를 가지고 이웃을 판단한다. 
      # 그래서 negative의 경우 destination node를 기준으로 이웃을 판단하게 된다.
      # neighbors: (n_source_nodes, n_neighbors)
      ...
      # 4. Attn해서 source_nodes의 최종 임베딩 구하기
      edge_time_embeddings = self.time_encoder(edge_deltas_torch)
      # self.edge_features: (n_inter+1, n_features) # torch.Size([6633, 31])
      # edge_idxs: (n_source_nodes, n_neighbors) # torch.Size([1200, 10])
