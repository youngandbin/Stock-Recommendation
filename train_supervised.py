...
from evaluation.evaluation import eval_node_classification, eval_recommendation
...
for i in range(args.n_runs):
  ...
  for epoch in range(args.n_epoch):
    ...
    for k in range(num_batch):
      ...
      sources_batch = train_data.sources[s_idx: e_idx] # shape: (100,) # [1 2 3 4 3 2 2 5 ...] # <class 'numpy.ndarray'>
      destinations_batch = train_data.destinations[s_idx: e_idx] # shape: (100,) # [118 119 120 121 122 ...] # <class 'numpy.ndarray'>
      timestamps_batch = train_data.timestamps[s_idx: e_idx]
      edge_idxs_batch = full_data.edge_idxs[s_idx: e_idx]
      labels_batch = train_data.labels[s_idx: e_idx] # shape: (100,) # [0 0 0 0 0 0 0 0 ...] # <class 'numpy.ndarray'>
      ...
      neg_scores = torch.sum(source_embedding * _, dim=1)
      # loss 계산
    ...
    #                                    n_neighbors=NUM_NEIGHBORS)
    val_auc = eval_recommendation(tgn, val_data, val_data.timestamps, full_data.edge_idxs, BATCH_SIZE,
                                  n_neighbors=NUM_NEIGHBORS)
    print('@@@@@@@@@@@@@@@@@@@@@@')
    print(val_auc)
